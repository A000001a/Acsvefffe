...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="财务名称" prop="caiwumingcheng">
            <el-input v-model="ruleForm.caiwumingcheng" 
                placeholder="财务名称" clearable :disabled=" false  ||ro.caiwumingcheng"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="收入金额" prop="shourujine">
			<el-input-number v-model="ruleForm.shourujine" placeholder="收入金额" :disabled=" false ||ro.shourujine"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="支出金额" prop="zhichujine">
			<el-input-number v-model="ruleForm.zhichujine" placeholder="支出金额" :disabled=" false ||ro.zhichujine"></el-input-number>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="财务详情" prop="caiwuxiangqing">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="财务详情"
              v-model="ruleForm.caiwuxiangqing">
            </el-input>
          </el-form-item>
  ...
  export default {
    data() {
      ...
      return {
        ...
        baseUrl: '',
        ro:{
				caiwumingcheng : false,
				dengjishijian : false,
				shourujine : false,
				zhichujine : false,
				caiwuxiangqing : false,
				zuzhizhanghao : false,
				zuzhixingming : false,
        },
        type: '',
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='caiwumingcheng'){
              this.ruleForm.caiwumingcheng = obj[o];
              this.ro.caiwumingcheng = true;
              continue;
            }
            if(o=='dengjishijian'){
            ...
            if(o=='caiwuxiangqing'){
              this.ruleForm.caiwuxiangqing = obj[o];
              this.ro.caiwuxiangqing = true;
              continue;
            }
            if(o=='zuzhizhanghao'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zuzhicaiwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zuzhicaiwu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zuzhicaiwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
