...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="组织电话" prop="zuzhidianhua">
            <el-input v-model="ruleForm.zuzhidianhua" 
                placeholder="组织电话" clearable :disabled=" false  ||ro.zuzhidianhua"></el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          zuzhixingming: [
            { required: true, message: '组织姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          zuzhidianhua: [
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='zuzhidianhua'){
              this.ruleForm.zuzhidianhua = obj[o];
              this.ro.zuzhidianhua = true;
              continue;
            }
            if(o=='touxiang'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zuzhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zuzhi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zuzhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
