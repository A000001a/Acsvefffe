...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="干部电话" prop="ganbudianhua">
            <el-input v-model="ruleForm.ganbudianhua" 
                placeholder="干部电话" clearable :disabled=" false  ||ro.ganbudianhua"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="所属组织" prop="zuzhimingcheng">
            <el-input v-model="ruleForm.zuzhimingcheng" 
                placeholder="所属组织" clearable :disabled=" false  ||ro.zuzhimingcheng"></el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='ganbudianhua'){
              this.ruleForm.ganbudianhua = obj[o];
              this.ro.ganbudianhua = true;
              continue;
            }
            if(o=='touxiang'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`ganbu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('ganbu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`ganbu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
